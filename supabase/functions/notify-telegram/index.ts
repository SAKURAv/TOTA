...
// بتستقبل استدعاء مباشر من كود الموقع (assets/js/telegram-notify.js)
// أو من برنامج الأدمن (SupabaseService.notifyTelegramEvent) بعد أي
// عملية مهمة تنجح فعليًا، وتبعت رسالة فورية في الخاص لبوت تليجرام
// بتاع الأدمن. (كانت بتشتغل عن طريق Database Webhook قديمًا، لكن
// اتغيّرت لاستدعاء مباشر بسبب مشكلة إعداد من ناحية Supabase منعت
// سكيما supabase_functions من الظهور في بعض المشاريع — الفنكشن نفسها
// مبتفرقش، بتستقبل نفس شكل الـ payload (table/type/record/old_record)
// من أي مصدر.)
//
// الأحداث اللي بتتغطى دلوقتي:
//   - orders (insert) حيث status = 'placed'
//       → المستخدم ضغط "اطلب الآن" في السلة، أو الأدمن ضاف أوردر
//         يدوي من البرنامج: بتتبعت رسالة واحدة كاملة فيها كل منتجات
//         الأوردر + العنوان + سعر التوصيل + الإجمالي + رقم الهاتف +
//         الملاحظة الإضافية لو موجودة.
//   - orders (update) حيث delivery_status بيتغير لـ 'delivered'
//       → الأدمن علّم الأوردر إنه "تم التسليم" من البرنامج.
//   - account_delete_requests (insert) → طلب حذف حساب جديد.
//
// ------------------------------------------------------------
// ⚠️ لو الرسايل مش وصلة، اتأكد من:
//   1) الـ secrets: TELEGRAM_BOT_TOKEN, TELEGRAM_ADMIN_CHAT_ID (أو
//      TELEGRAM_ADMIN_CHAT_IDS لأكتر من أدمن), SUPABASE_URL,
//      SUPABASE_SERVICE_ROLE_KEY متظبطة (supabase secrets set...).
//      من غيرهم الفنكشن هترجع 200 (عشان الكود اللي بينادي عليها
//      ميعتبرش نفسه فشل) لكن مش هتبعت أي رسالة فعليًا، وهتسجل تفصيل
//      السبب في function logs (Dashboard → Edge Functions →
//      notify-telegram → Logs).
//   2) إن كل أدمن من اللي أرقامهم في TELEGRAM_ADMIN_CHAT_IDS بدأ
//      محادثة مع البوت شخصيًا قبل كده (تليجرام مش بيسمح لبوت يبعت
//      لحد لسه ما بدأش معاه محادثة).
//   3) إن الفنكشن نفسها منشورة (Edge Functions → notify-telegram
//      لازم تكون ظاهرة في القايمة).
// ============================================================
